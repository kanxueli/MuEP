...
# You need to specify the url before running the client
url = '<your_url>'
...
def insert_element(queue, new_element):  # Queue inserting
    if len(queue) == 4:
        queue.popleft()
...
# You need to specify the testing environment ('AlfredTWEnv' or 'AlfredThorEnv')
env_type = "AlfredTWEnv"  # 'AlfredTWEnv' or 'AlfredThorEnv'
if env_type == "AlfredThorEnv":
    env = ThorEnvironment()

# You need to specify the testing scenario (seen or unseen)
# unseen_valid_path = "valid_unseen.json"
unseen_valid_path = "unseen_valid_new_generation.json"
seen_valid_path = "valid_seen.json"
env_file_list = []
with open(unseen_valid_path,'r') as load_f:
    ...
    if action_data:
        while cur_step < env_steps:
            if file_transf_test_flag:
                ...
                text_input = construt_prompt(text_input, new_action=action_data, observation=obs)
                feedback_data.history = text_input
                # feedback_data.observation = "What is unusual about this image?"
                if env_type == 'AlfredThorEnv':
                    feedback_data.image = json.dumps(env.get_frame_image().tolist())
